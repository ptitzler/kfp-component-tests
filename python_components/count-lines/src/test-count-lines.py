...
from count_lines import count # noqa I100
...
count_lines_op = create_component_from_func(
    func=count,
    base_image=None,
    packages_to_install=None,
    output_component_file='count_lines_component.yaml')
...
#
@dsl.pipeline(
    name='count lines pipeline',
    description='An example pipeline that counts lines in a file.'
)
def count_pipeline(input_file):
    # Passes a pipeline parameterto the `count_lines_op` factory
    # function.
    count_task = count_lines_op(input_file) # noqa F841
...
if __name__ == "__main__":
    ...
    run_name = f'count-lines-python-func-run-{datetime.now().strftime("%m%d%H%M%S")}'
    ...
    run = client.create_run_from_pipeline_func(count_pipeline,
                                               run_name=run_name,
                                               namespace=namespace,
                                               arguments=arguments)
    print(run)
