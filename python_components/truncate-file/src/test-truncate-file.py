...
from truncate_file import truncate # noqa I100
...
truncate_op = create_component_from_func(
    func=truncate,
    base_image=None,
    packages_to_install=None,
    output_component_file='truncate_file_component.yaml')
...
#
@dsl.pipeline(
    name='Truncate file pipeline',
    description='An example pipeline that truncates a file.'
)
def truncate_pipeline(file_in, threshold):
    # Passes a pipeline parameterto the `truncate_op` factory
    # function.
    truncate_task = truncate_op(file_in, threshold) # noqa F841
...
if __name__ == "__main__":
    ...
    run_name = f'truncate-file-python-func-run-{datetime.now().strftime("%m%d%H%M%S")}'
    print(f'Creating run {run_name} from pipeline...')

    # Specify argument values for your pipeline run.
    arguments = {'file_in': 'line1\nline2\nline3\nline4\nline5',
                 'threshold': 2}

    # Compile, upload, and submit this pipeline for execution.
    run = client.create_run_from_pipeline_func(truncate_pipeline,
                                               run_name=run_name,
                                               namespace=namespace,
                                               arguments=arguments)
    print(run)
